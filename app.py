...
if st.button("Predict Risk"):
    ...
    input_data["HasCrCard"] = input_data["HasCrCard"].map({"Yes": 1, "No": 0}).astype(int)
    input_data["IsActiveMember"] = input_data["IsActiveMember"].map({"Yes": 1, "No": 0}).astype(int)

    encoded_features = encoder.transform(input_data[["Geography", "Gender"]])
    new_cols = encoder.get_feature_names_out(["Geography", "Gender"])
    encoded_df = pd.DataFrame(encoded_features, columns=new_cols, index=input_data.index)
    
    final_input = pd.concat([input_data.drop(["Geography", "Gender"], axis=1), encoded_df], axis=1)
